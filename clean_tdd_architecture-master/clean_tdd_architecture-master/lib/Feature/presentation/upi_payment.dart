...
class MerchantScreen extends State<MerchantApp> {
  ...
  List<String> environmentList = <String>['UAT_SIM','SANDBOX', 'PRODUCTION'];
...

  {
    ...
    ;
    String bash64Body=base64Encode(utf8.encode(jsonEncode(requestData)));
    checksum = '${sha256.convert(utf8.encode(bash64Body + endPoint + saltKey)).toString()} + ### + saltIndex';
    return bash64Body;
}
  ...
  void startTransaction() async {
    try {
      PhonePePaymentSdk.startTransaction(body, callback, checksum, endPoint)
          .then((response) =>
      {
        setState(() {
          if (response != null) {
